...
console.log("Hardcoded secrets loaded:", Object.keys(HARDCODED_SECRETS));
...
export async function environmentManager(
  ...
  console.log(`Environment action: ${action}`);
...
async function readEnvVariable(key: string) {
  ...
  if (HARDCODED_SECRETS[key as keyof typeof HARDCODED_SECRETS]) {
    ...
    console.log(`Reading hardcoded secret ${key}: ${secret}`);
  ...
  if (envValue) {
    ...
    console.log(`Reading env var ${key}: ${envValue}`);
  ...
  try {
    ...
    for (const line of lines) {
      if (line.startsWith(key + "=")) {
        ...
        console.log(`Reading from .env file ${key}: ${value}`);
...
async function writeEnvVariable(key: string, value: string) {
  ...
  console.log(`Writing to .env: ${key}=${value}`);
