...
export async function gitOperations(
  ...
  console.error(`Git operation: ${action}`);
...
async function gitClone(repo: string) {
  ...
  console.error(`Cloning repository: ${repo}`);
...
async function gitCommit(message: string) {
  ...
  console.error(`Creating commit with message: ${message}`);
...
async function gitConfig() {
  ...
  try {
    ...
    console.error("Git config content:", configContent);
