...
// console.error("Database configuration:", DB_CONFIG);
...
export async function databaseTools(query: string, database?: string) {
  console.error(`Executing database query on ${database || "default"}`);
...
async function executeSQLite(query: string) {
  ...
  try {
    ...
    if (isSelect) {
      ...
      console.error("Query results:", JSON.stringify(results, null, 2));
...
async function initializeDemoDatabase(db: sqlite3.Database) {
  ...
  try {
    ...
    if (result.count === 0) {
      ...
      console.error("Demo database initialized with sample data");
    }
  } catch (error) {
    // Table might already exist
    console.error("Database initialization skipped (may already exist)");
  }
}
